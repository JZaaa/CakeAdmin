...
/**
 * 无法使用 url rewrite 时,Auth组件会出现无法获取登录信息的问题，请用此组件代替auth组件
 *
 * 该组件基于Auth组件修改，仅替换了storage的操作，配置时请删除 'storage' 配置项，并用 'sessionKey' 代替(sessionKey为储存关键字)
 *
 * 其他所有配置与操作与 Auth 相同
 *
...
class SimpleAuthComponent extends Component
{
     ...
     *   that should be set to all authorization objects using the SimpleAuthComponent::ALL key:
     *
     *   ```
     *   $this->Auth->setConfig('authorize', [
     *      SimpleAuthComponent::ALL => [
     ...
     *   While SimpleAuthComponent does not handle post-logout redirection, a redirect URL
     *   will be returned from `SimpleAuthComponent::logout()`. Defaults to `loginAction`.
     *
    ...
    protected $_user;
    /**
     * Other components utilized by SimpleAuthComponent
     *
     * @var array
     */
     ...
     *
     * @var array
     * @see \Cake\Controller\Component\SimpleAuthComponent::allow()
     */
    ...
    public function constructAuthorize()
    {
        ...
        $global = [];
        if (isset($authorize[SimpleAuthComponent::ALL])) {
            $global = $authorize[SimpleAuthComponent::ALL];
            unset($authorize[SimpleAuthComponent::ALL]);
        }
    ...
    /**
     * Similar to SimpleAuthComponent::user() except if user is not found in
    ...
    public function constructAuthenticate()
    {
        ...
        $global = [];
        if (isset($authenticate[SimpleAuthComponent::ALL])) {
            $global = $authenticate[SimpleAuthComponent::ALL];
            unset($authenticate[SimpleAuthComponent::ALL]);
        }
    ...
    {
        if ($name === 'sessionKey') {
            return $this->_config['sessionKey'];
        }
    ...
    protected function _readUser()
    {
        if ($this->_user !== null) {
            return $this->_user ?: null;
        }

        $this->_user = $this->session->read($this->_config['sessionKey']) ?: false;
    ...
    protected function _writeUser($user)
    {
        $this->session->write($this->_config['sessionKey'], $user);
    }
