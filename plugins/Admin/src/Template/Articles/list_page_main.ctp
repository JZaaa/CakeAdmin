...
<div class="bjui-pageContent">
    <div style="float:left; width:200px;">
        <ul id="listpagemain-tree" class="ztree" data-toggle="ztree" data-expand-all="true" data-on-click="listpagemain_layout">
            ...
            <li data-id="<?php echo $item['id']?>" data-href="<?php if (!empty($item['href'])) echo $this->Url->build($item['href']); else echo $this->Url->build(['plugin' => 'Admin', 'controller' => 'Articles', 'action' => 'manage', $item['id'], $item['type'], 'listpagemainbox'])?>" data-divid="#listpagemainbox" data-pid="<?php echo $item['parent_id']?>"><?php echo $item['name']?></li>
            <?php endforeach;?>
        </ul>
    </div>
